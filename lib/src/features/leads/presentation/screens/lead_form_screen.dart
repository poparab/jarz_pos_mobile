...
import '../widgets/lead_maps_import_card.dart';
...
class LeadFormScreen extends ConsumerStatefulWidget {
  const LeadFormScreen({super.key, this.existing, this.mapsPreviewResolver});

  final Lead? existing;

  /// Test seam and optional host override. Normal screens use the authenticated
  /// [LeadsRepository] resolver.
  final LeadMapsPreviewResolver? mapsPreviewResolver;

  static const saveButtonKey = ValueKey('lead_form_save');

  static Key fieldKey(String name) => ValueKey('lead_form_$name');
...
class _LeadFormScreenState extends ConsumerState<LeadFormScreen> {
  ...
  void initState() {
    ...
    _c = {
      ...
      'company_name': TextEditingController(text: e?.companyName ?? ''),
      'phone': TextEditingController(text: e?.phone ?? ''),
      'mobile': TextEditingController(text: e?.mobileNo ?? ''),
      'email': TextEditingController(text: e?.emailId ?? ''),
    ...
    _source = e?.source.trim().isEmpty == false ? e!.source : null;
    _territory = e?.territory.trim().isEmpty == false ? e!.territory : null;
    _maps = LeadMapsValue(
      mapsUrl: e?.mapsUrl ?? '',
      latitude: e?.latitude,
  ...
  String? _addressValidationError(
    ...
    if (!anyValue) return null;
    if (src['line1']!.text.trim().isEmpty ||
        src['city']!.text.trim().isEmpty ||
        src['country']!.text.trim().isEmpty) {
      return context.l10n.leadFormAddressRequired(title);
    }
    return null;
  }

  /// Apply Maps suggestions at response time, and only into fields that are
  /// still blank then. A rep may start typing while a lookup is in flight; that
  /// fresh input always wins over Google's suggestion.
  void _applyMapsPreview(LeadMapsPreview preview) {
    void fill(TextEditingController controller, String? suggestion) {
      final value = suggestion?.trim() ?? '';
      if (value.isNotEmpty && controller.text.trim().isEmpty) {
        controller.text = value;
        _mapsAutofilled[controller] = value;
      }
    }

    fill(_c['lead_name']!, preview.placeName);
  ...
  void _onMapsChanged(LeadMapsValue value) {
    if (value.mapsUrl != _maps.mapsUrl) {
      for (final entry in _mapsAutofilled.entries) {
        // Remove only untouched suggestions. Anything the rep has changed is
        // manual input now and must survive replacing or clearing the link.
  ...
  Future<void> _save() async {
    ...
    try {
      final payload = <String, dynamic>{
        ...
        // Always send all three fields. This makes Clear meaningful on edit,
        // and prevents a replacement link that did not resolve from retaining
        // the previous place's coordinates.
        'maps_url': _maps.mapsUrl,
        'latitude': _maps.latitude,
        'longitude': _maps.longitude,
      ...
      final name = await repo.saveLead(
        payload,
        name: _savedLeadName ?? widget.existing?.name,
      );
      // If an address write fails after a new Lead was created, retry against
      // that same record instead of creating a duplicate.
      if (name.trim().isNotEmpty) _savedLeadName = name.trim();
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text(context.l10n.leadFormSaved)));
      context.pop();
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(
    ...
    final name = await showDialog<String>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: TextField(
          ...
          autofocus: true,
          decoration: InputDecoration(labelText: l10n.leadFormCategoryName),
        ),
        actions: [
          TextButton(
          ...
          children: [
            _card(context.l10n.leadMapsCardTitle, [
              LeadMapsImportCard(
                initialValue: _maps,
                resolver:
                    widget.mapsPreviewResolver ??
                    ref.read(leadsRepositoryProvider).previewMapsLink,
                onChanged: _onMapsChanged,
                onImported: _applyMapsPreview,
              ),
            ]),
                ...
                _Field.full(
                  ...
                  Material(
                    ...
                    child: SwitchListTile.adaptive(
                      ...
                      title: Text(
                        context.l10n.leadFormSpecialty,
            ...
            _card(context.l10n.leadFormCardContact, [
              _grid([
                ...
                _Field.half(_text('facebook', context.l10n.leadFieldFacebook)),
              ]),
            ]),
            const SizedBox(height: 16),
            _card(
            ...
            FilledButton.icon(
              key: LeadFormScreen.saveButtonKey,
  ...
  List<Widget> _addressFields(Map<String, TextEditingController> src) => [
    _grid([
      _Field.full(_addrField(src, 'line1', context.l10n.leadFieldAddressLine1)),
      _Field.full(_addrField(src, 'line2', context.l10n.leadFieldAddressLine2)),
      _Field.half(_addrField(src, 'city', context.l10n.leadFieldCity)),
      _Field.half(_addrField(src, 'state', context.l10n.leadFieldState)),
      _Field.half(_addrField(src, 'country', context.l10n.leadFieldCountry)),
      _Field.half(_addrField(src, 'pincode', context.l10n.leadFieldPincode)),
      _Field.half(
        _addrField(
          src,
          'phone',
  ...
  Widget _grid(List<_Field> fields) {
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        final halfW = twoCol ? ((maxW - spacing) / 2).floorToDouble() : maxW;
        return Wrap(
          spacing: spacing,
          runSpacing: spacing,
          children: [
  ...
  Widget _text(
    ...
    return TextFormField(
      key: LeadFormScreen.fieldKey(key),
      controller: _c[key],
      keyboardType: keyboard,
      style: LeadsTheme.body,
      decoration: _dec(label),
    ...
    return sourcesAsync.when(
      data: (sources) {
        final options = <String>{
          if (_source != null && _source!.isNotEmpty) _source!,
          ...sources.where((source) => source.trim().isNotEmpty),
        };
        return DropdownButtonFormField<String>(
      ...
      data: (territories) {
        ...
        return DropdownButtonFormField<String>(
          ...
          items: [
            ...
            if (_territory != null &&
                _territory!.isNotEmpty &&
                !names.contains(_territory))
              DropdownMenuItem<String>(
                value: _territory,
                child: Text(_territory!),
              ),
            ...territories
                .where(
                  (territory) =>
                      (territory['name']?.toString() ?? '').isNotEmpty,
                )
                .map<DropdownMenuItem<String>>((territory) {
          ...
          onChanged: (v) =>
              setState(() => _territory = v == null || v.isEmpty ? null : v),
        );
      },
      loading: () => _loadingField(label),
      error: (_, _) => _loadingField(label, spinner: false),
          ...
          Expanded(
            child: DropdownButtonFormField<String?>(
              ...
              items: [
                ...
                if (value != null &&
                    value!.isNotEmpty &&
                    !categories.any((category) => category.name == value))
                  DropdownMenuItem<String?>(value: value, child: Text(value!)),
                for (final cat in categories)
                  DropdownMenuItem<String?>(
                    value: cat.name,
                    child: Text(
                      cat.categoryName.isEmpty ? cat.name : cat.categoryName,
                    ),
                  ),
              ],
              onChanged: onChanged,
