...
/// The posting date the batch tab shows when the operator has not picked one.
///
...
/// came from `policy.today()`, the SERVER's day. Two clocks, one date, and the
/// disagreement only appears on a tablet whose clock is off: exactly the device
/// nobody is looking at.
///
/// These tests pin the policy to a day the device clock cannot be on, so a
/// regression shows up as the machine's real date appearing in the bar.
void main() {
  testWidgets('defaults the posting date to the server day, not the device day',
      (tester) async {
    // A day no machine running this test is on, so the assertion cannot pass
    // by coincidence.
  ...
  testWidgets('a server day AHEAD of the device does not read as backdated',
    ...
    await _pumpTab(
      tester,
      policy: ProductionPolicy(serverDate: day),
    );
  ...
  await tester.pumpWidget(
    ProviderScope(
      overrides: [
        ...
        // and one MaterialOptions per line on every build.
      ...
      child: MaterialApp(
        ...
        home: const Scaffold(body: ProductionBatchTab()),
      ),
    ),
  );
